...
//
//
// $Id: L4Scintillation.hh 108508 2018-02-15 15:54:35Z gcosmo $
//
// 
////////////////////////////////////////////////////////////////////////
...
#ifndef L4Scintillation_h
#define L4Scintillation_h 1
...
class L4Scintillation : public G4VRestDiscreteProcess
{
...
public:
	...
	explicit L4Scintillation(const G4String& processName = "Scintillation",
                                 G4ProcessType type = fElectromagnetic);
	~L4Scintillation();

private:

        L4Scintillation(const L4Scintillation &right) = delete;

        //////////////
        // Operators
        //////////////

        L4Scintillation& operator=(const L4Scintillation &right) = delete;
...
public:
        ...
        // L4Scintillation Process has both PostStepDoIt (for energy 
        ...
        // If set, the L4Scintillation process expects the user to have
        ...
        // Call by the user to set the L4ScintillationTrackInformation
        // to scintillation photon track

        G4bool GetScintillationTrackInfo() const;
        // Return the boolean for whether or not the
        // L4ScintillationTrackInformation is set to the scint. photon track
...
inline
void L4Scintillation::SetTrackSecondariesFirst(const G4bool state)
{
        fTrackSecondariesFirst = state;
}

inline
G4bool L4Scintillation::GetTrackSecondariesFirst() const
{
        return fTrackSecondariesFirst;
}

inline
void L4Scintillation::SetFiniteRiseTime(const G4bool state)
{
        fFiniteRiseTime = state;
}

inline 
G4bool L4Scintillation::GetFiniteRiseTime() const
{
        return fFiniteRiseTime;
}

inline
void L4Scintillation::SetScintillationYieldFactor(const G4double yieldfactor)
{
        fYieldFactor = yieldfactor;
}

inline
G4double L4Scintillation::GetScintillationYieldFactor() const
{
        return fYieldFactor;
}

inline
void L4Scintillation::SetScintillationExcitationRatio(const G4double ratio)
{
        fExcitationRatio = ratio;
}

inline
G4double L4Scintillation::GetScintillationExcitationRatio() const
{
        return fExcitationRatio;
}

inline
G4PhysicsTable* L4Scintillation::GetSlowIntegralTable() const
{
        return fSlowIntegralTable;
}

inline
G4PhysicsTable* L4Scintillation::GetFastIntegralTable() const
{
        return fFastIntegralTable;
}

inline
void L4Scintillation::AddSaturation(G4EmSaturation* sat)
{
        fEmSaturation = sat;
}

inline
void L4Scintillation::RemoveSaturation()
{
        fEmSaturation = nullptr;
}

inline
G4EmSaturation* L4Scintillation::GetSaturation() const
{
        return fEmSaturation;
}

inline
G4bool L4Scintillation::GetScintillationByParticleType() const
{
        return fScintillationByParticleType;
}

inline
void L4Scintillation::SetScintillationTrackInfo(const G4bool trackType)
{
        fScintillationTrackInfo = trackType;
}

inline
G4bool L4Scintillation::GetScintillationTrackInfo() const
{
        return fScintillationTrackInfo;
}

inline
void L4Scintillation::SetStackPhotons(const G4bool stackingFlag)
{
        fStackingFlag = stackingFlag;
}

inline
G4bool L4Scintillation::GetStackPhotons() const
{
        return fStackingFlag;
}

inline
G4int L4Scintillation::GetNumPhotons() const
{
        return fNumPhotons;
}
...
inline
G4double L4Scintillation::single_exp(G4double t, G4double tau2)
{
         return std::exp(-1.0*t/tau2)/tau2;
}

inline
G4double L4Scintillation::bi_exp(G4double t, G4double tau1, G4double tau2)
{
         return std::exp(-1.0*t/tau2)*(1-std::exp(-1.0*t/tau1))/tau2/tau2*(tau1+tau2);
}

#endif /* L4Scintillation_h */
