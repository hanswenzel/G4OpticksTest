...
ckm-go
    builds and runs the G4OpticksTest executable
ckm--
    just runs the G4OpticksTest executable

Objectives
------------

G4OpticksTest is really intended to feature **Minimal** usage of Opticks. 
...
* G4OpticksTest is NOT a place for debugging, the 2nd executable that 
  adopts the G4OpticksTest geocache and gensteps is where debugging is done

* moving to not using PLOG in G4OpticksTest makes a stark contrast
...
ckm-key(){ echo G4OpticksTest.X4PhysicalVolume.World.$(ckm-dig) ; }
ckm-key-export(){ export OPTICKS_KEY=$(ckm-key) ;  }
ckm-indexer-test(){   OPTICKS_KEY=$(ckm-key) IndexerTest --envkey ; }


ckm-idpath(){ echo $LOCAL_BASE/opticks/geocache/G4OpticksTest_World_g4live/g4ok_gltf/$(ckm-dig)/1 ; }
...
{ 
    ...
    G4OpticksTest  # NB the Opticks is embedded via G4OK : so commandline doesnt get thru 
}
...
Uncomment the relevant B slot then run ckm-so::
    ...
    pwd /usr/local/opticks/geocache/G4OpticksTest_World_g4live/g4ok_gltf/792496b5e2cc08bdf5258cc12e63de9f/1
...
ckm-addr2line()
{
    ...
    PATH=/usr/bin lldb $(which G4OpticksTest) -o "source list -a $addr"  --batch
}
...
ckm-- OR ckm-go  
    G4OpticksTest : geant4 example app, with genstep and photon collection
